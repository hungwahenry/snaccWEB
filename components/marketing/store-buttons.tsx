...
function StoreButton({
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="flex flex-1 items-center justify-center gap-2.5 rounded-2xl bg-foreground px-3.5 py-2.5 text-background transition-transform hover:scale-[1.02] sm:flex-initial sm:gap-3 sm:px-5 sm:py-3"
    >
      {logo}
      <span className="flex flex-col text-left leading-tight whitespace-nowrap">
        ...
        <span className="text-sm font-semibold sm:text-base">{bottom}</span>
      </span>
    </a>
  )
}
