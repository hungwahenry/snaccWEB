...
export function toSuspensionInput(draft: SuspensionDraft, note?: string) {
  const days = DURATIONS.find((option) => option.value === draft.duration)?.days ?? null
    ...
    <>
      <div className="flex gap-3">
        <Field className="flex-1">
          ...
          <Select
            ...
            onValueChange={(next) => next && onChange({ ...value, reasonId: next })}
          >
            <SelectTrigger>
        ...
        <Field className="w-40">
          ...
          <Select
            ...
            onValueChange={(next) => next && onChange({ ...value, duration: next })}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
      ...
      {chosen ? (
        <div className="bg-muted rounded-lg px-3 py-2">
          ...
          <p className="text-muted-foreground mt-1 text-xs">{chosen.description}</p>
        </div>
      ) : (
        <p className="text-muted-foreground text-xs">
          Without a reason they get generic wording. Pick one so they know what happened.
        </p>
      )}
    </>
  )
